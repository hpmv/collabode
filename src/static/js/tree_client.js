...
function getTreeClient(comet, hilite) {
  $(document).ready(function() {
    ...
    comet.setOnChannelStateChange(function(state, info) {
      if (state == "CONNECTED") {
        $("#connstatusconnecting").css('display', 'none');
        $("#connstatusdisconnected").css('display', 'none');

        // Request entire tree from server.  The current_file is used so that server knows
        // which folders should be marked as open.
        setTimeout(function() {comet.sendMessage({ type : "TREE_INIT_REQUEST", current : current_file}); }, 0);
    ...
    comet.setOnMessage("TREE_INIT", function(msg) {
    ...
    comet.setOnMessage("TREE_ADD", function(msg) {
      (msg.data).forEach(function(item) {
    ...
    $("#treecontainer").bind("open_node.jstree", function(e, data) {
      comet.sendMessage({ type : "TREE_FOLDER_OPENED", path : $("#treecontainer").jstree("get_path", data.rslt.obj), current : current_file });
    });
    ...
    $("#treecontainer").bind("close_node.jstree", function(e, data) {
      ...
      // Send message
      comet.sendMessage({ type : "TREE_FOLDER_CLOSED", path : $("#treecontainer").jstree("get_path", data.rslt.obj) });
    });
  });
...
$(document).ready(function(){
  ...
  var cometClient = getCometClient(user, { });
  getTreeClient(cometClient, true);
});
